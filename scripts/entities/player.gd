...
##
## Handles tank movement, turret rotation, shooting, and damage.
## Interacts with GameManager for stats.
...
func get_mouse_world_position() -> Vector3:
	...
	var ray_end: Vector3 = ray_origin + camera.project_ray_normal(mouse_pos) * 2000 # 2000 is the 'range'
...
func _physics_process(delta: float) -> void:
	...
	if target_pos != Vector3.ZERO:
		...
		var turret_speed: float = GameManager.player_turret_speed.get_value()
		var rotation_change: float = clamp(angle_diff, -turret_speed * delta, turret_speed * delta)
	...
	if input_dir:
		...
		var pd_speed: float = GameManager.player_speed.get_value()
