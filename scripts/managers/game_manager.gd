...
func trigger_upgrade_selection() -> void:
    ...
    if ui_scene:
        ...
        if ui_instance.has_method("set_options"):
            # We need to gather behaviors from the MAIN vehicle (or all?) for the "Smart" upgrades
            # For now, let's look at Main Vehicle behaviors
            var behaviors_for_smart_check: Array[ProjectileBehavior] = []
            if is_instance_valid(main_vehicle):
                behaviors_for_smart_check = main_vehicle.active_projectile_behaviors
            
            var options: Array[UpgradeData] = upgrade_pool.pick_random_upgrades(3, behaviors_for_smart_check)
...
func apply_upgrade(upgrade: UpgradeData) -> void:
    ...
    if upgrade.target_stat != "":
        ...
        # Removing "player_" prefix if present, though cleaner to update UpgradeData resources eventually.
    ...
    # 2. Apply Granted Behaviors (Local - Default to Main Vehicle)
        ...
        if is_instance_valid(main_vehicle):
            ...
            if behavior is Script:
                ...
                if instance is ProjectileBehavior:
                    main_vehicle.active_projectile_behaviors.append(instance)
            elif behavior is ProjectileBehavior:
                main_vehicle.active_projectile_behaviors.append(behavior)
        else:
            print("GameManager: Cannot apply behavior - No Main Vehicle!")

    # 3. Apply Behavior Upgrades (Smart - Default to Main Vehicle)
    if upgrade.target_behavior_script and upgrade.target_behavior_stat != "":
        if is_instance_valid(main_vehicle):
            var target_instance: ProjectileBehavior = null
            for b in main_vehicle.active_projectile_behaviors:
